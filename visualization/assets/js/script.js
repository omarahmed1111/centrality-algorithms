function createNetworkVisualization(error, graph, centralityMeasures) {
  ...
  function updateCentralityMeasure(centralityMeasure) {
    nodes
      ...
      .duration(250)
      .attr('r', function(d) {
        if (centralityMeasure == 'degree') {
          return circleRadiusScale(d.centrality[centralityMeasure] / maxDegree);
        } else if (centralityMeasure == 'betweenness') {
          return circleRadiusScale(
            d.centrality[centralityMeasure] / maxBetweenness
          );
        } else {
          return circleRadiusScale(d.centrality[centralityMeasure]);
        }
      });
    nodes.selectAll('title').text(function(d) {
      return formatCentrality(d.centrality[centralityMeasure]);
    });
  }
  ...
  function createCentralityTable() {
    var table = d3.select('table').style('width', width);
  ...
  function highlightNode(node) {
    ...
    function highlightElement(selector, node) {
      ...
      if (node)
        elements.classed('highlight', function(d) {
          return d.id === node.id;
        });
    }
